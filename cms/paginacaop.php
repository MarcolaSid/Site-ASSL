<?
 $pagina = $_GET["pagina"];
 $sql = mysql_query("SELECT projeto.id FROM projeto JOIN foto ON foto.id_projeto = projeto.id GROUP BY projeto.titulo, foto.id_projeto") or die(mysql_error());
 $Qtde = 4;
 ...
 $inicio = $limit * $Qtde;
 $sql = mysql_query("SELECT * FROM projeto ORDER BY id DESC Limit $inicio, $quantidade") or die(mysql_error());  
...
?>
 ...
 <?
   ...
   echo " Ir para a p&aacute;gina &nbsp; <input type='text' class='text-input paginacao-input' id='pagina' size='3' maxlength='3' value=\"$pagina\" />
<input type='button' class='button' value='ok' onclick=\"fctTrocaPagina('$PHP_SELF',$Paginas)\" />";
 ?>
 </form>
